...
[Modules](/steps/module) have their own context that is independent from any tests that include them. They can also be passed data via parameters and inputs.

[Modules](/steps/module) do automatically inherit all values on `env` from its parent test at the **time of execution**. This means that if you modify `env` inside a [Module](/steps/module), it will not affect the parent test.
