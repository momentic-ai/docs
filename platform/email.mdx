...
This function will automatically retry until a matching email is found or the timeout is exhausted (if the timeout parameter is omitted, the default is 10 seconds).
...
<Tabs>
  <Tab title="Function Signature">
    ```ts
    async function fetchLatestEmail(params: {
