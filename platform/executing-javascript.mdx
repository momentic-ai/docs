...
2. **`{{}}` expressions**: String fields in all steps can contain JavaScript expressions inside curly brackets. For example, the value of a [Type](/steps/type) step can be `{{ env.USERNAME }}@gmail.com` .
...
- [`axios`](https://github.com/axios/axios): Send HTTP/HTTPS requests to other servers
- `assert`: Execute assertions based on other variables and constants
- [`faker`](https://fakerjs.dev/): Generate mock data easily
- [`moment`](https://momentjs.com/): Create and manipulate `Date` objects
